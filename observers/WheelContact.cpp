...
void WheelContact::observe(const double torque,
                           const double velocity) noexcept {
  const double prev_velocity = velocity_;
  velocity_ =
      low_pass_filter(velocity_, params_.cutoff_period, velocity, params_.dt);
  const double new_acceleration = (velocity_ - prev_velocity) / params_.dt;
  abs_acceleration_ = low_pass_filter(abs_acceleration_, params_.cutoff_period,
                                      std::abs(new_acceleration), params_.dt);
  abs_torque_ = low_pass_filter(abs_torque_, params_.cutoff_period,
                                std::abs(torque), params_.dt);
